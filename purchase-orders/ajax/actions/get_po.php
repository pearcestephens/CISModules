...
/**
 * Action: po.get_po
 * Returns PO header + line items for hydration.
 * URL: https://staff.vapeshed.co.nz/modules/purchase-orders/ajax/handler.php
 */

declare(strict_types=1);
// tools.php already required by handler and provides po_pdo()

$ctx = $GLOBALS['__po_ctx'] ?? ['uid'=>0, 'request_id'=>''];
$po_id = (int)($_POST['po_id'] ?? 0);
if ($po_id <= 0) {
    po_jresp(false, ['code'=>'invalid_po_id','message'=>'po_id required'], 400);
}
try {
    $pdo = po_pdo();

    // Fetch header (with supplier/outlet names if present)
    $hdrSql = "SELECT po.purchase_order_id AS po_id,
             ...
             LEFT JOIN vend_outlets   vo ON vo.id = po.outlet_id
                 ...
                 LIMIT 1";
    $h = $pdo->prepare($hdrSql); $h->execute([$po_id]); $headerRow = $h->fetch();
    if (!$headerRow) {
        po_jresp(false, ['code'=>'not_found','message'=>'PO not found'], 404);
    }

    // Determine column names (compat across schemas)
    ...
    $qtyArrivedCol = po_has_column($pdo,'purchase_order_line_items','qty_arrived') ? 'qty_arrived' : 'qty_received';
    $receivedAtCol = po_has_column($pdo,'purchase_order_line_items','received_at') ? 'received_at' : null;

    // Build items query with safe joins if tables exist
    $hasVP = po_table_exists($pdo,'vend_products');
    $hasVI = po_table_exists($pdo,'vend_inventory');
    $select = [
        "poli.product_id",
        "poli.{$orderQtyCol} AS expected",
        "COALESCE(poli.{$qtyArrivedCol},0) AS received",
    ];
    if ($receivedAtCol) $select[] = "poli.{$receivedAtCol} AS received_at";
    if ($hasVP) { $select[] = 'vp.name AS product_name'; $select[] = 'vp.image_url'; $select[] = 'vp.sku'; }
    if ($hasVI) { $select[] = 'vi.inventory_level AS current_stock'; }
    $sel = implode(',', $select);
    $sql = "SELECT {$sel}
    ...
    if ($hasVI) { $stmt = $pdo->prepare($sql); $stmt->execute([$headerRow['outlet_id'], $po_id]); }
    else { $stmt = $pdo->prepare($sql); $stmt->execute([$po_id]); }
    ...
    while ($r = $stmt->fetch()) {
        ...
        $status = 'Pending';
        if ($received > 0) { $status = ($received >= $expected ? 'Complete' : 'Partial'); }
        $items[] = [
            'line_id' => $r['product_id'], // using product_id as line key for now
        ...
        $totExpected += $expected; $totReceived += $received;
    }

    $header = [
        'po_id' => (int)$headerRow['po_id'],
        'status' => ((int)($headerRow['status'] ?? 0) === 1 ? 'COMPLETED' : 'OPEN'),
        'supplier' => ['id'=> $headerRow['supplier_id'] ?? null, 'name'=> (string)($headerRow['supplier_name'] ?? 'Supplier')],
    ...
    error_log('[po.get_po] '.$e->getMessage());
    po_jresp(false, ['code'=>'internal_error','message'=>'Failed to load PO'], 500);
}

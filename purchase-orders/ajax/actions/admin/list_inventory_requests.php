<?php
/**
 * Filename: list_inventory_requests.php
 * Action: admin.list_inventory_requests
 * URL: https://staff.vapeshed.co.nz/modules/purchase-orders/ajax/handler.php
 * Purpose: Admin listing of inventory_adjust_requests with filters and pagination.
 */
...
try {
    $pdo = po_pdo();
    if (!po_table_exists($pdo,'inventory_adjust_requests')) po_jresp(true, ['rows'=>[], 'total'=>0, 'page'=>$page, 'size'=>$size]);
    $where = [];$args = [];
    if ($status !== '') { $where[] = 'status = ?'; $args[] = $status; }
    if ($outlet !== '') { $where[] = 'outlet_id = ?'; $args[] = $outlet; }
    $ws = $where ? ('WHERE '.implode(' AND ',$where)) : '';
    $sql = "SELECT request_id, outlet_id, product_id, delta, reason, source, status, idempotency_key, requested_by, requested_at, processed_at, error_msg
    ...
    error_log('[admin.list_inventory_requests] '.$e->getMessage());

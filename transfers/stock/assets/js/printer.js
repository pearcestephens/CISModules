...
(function(window, document){
  ...
  _initCarrier: function(){ var self=this; try{ fetch(this._opts.ajaxUrl+'?ajax_action=get_printers_config', { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, credentials:'same-origin', body: 'csrf_token='+(encodeURIComponent(this._opts.csrf||'')) }).then(function(r){ return r.json(); }).then(function(json){ if(!json || !json.success) return; var d=json.data||{}; var sel=document.getElementById('stx-carrier'); if(!sel) return; var hasNZ=!!d.has_nzpost, hasGSS=!!d.has_gss; // hide unavailable

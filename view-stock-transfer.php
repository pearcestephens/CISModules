...
/**
 * Legacy Stock Transfer pack page shim.
 * Forwards to the CIS module Stock Transfers READY view while keeping old URL working.
 * All inline CSS/JS has been migrated into module assets; this file remains a thin router.
 */
...
// Derive PACKONLY (default false unless explicitly requested)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // 3.5) Fast-path handlers to avoid legacy HTML leaking into JSON responses
  // Handle searchForProduct here with a direct DB query and exit cleanly
  if ($action === 'searchForProduct') {
    try {
      ...
      error_log('[legacy.view-stock-transfer.searchForProduct] ' . $e->getMessage());
      st_json_response([ 'success' => false, 'error' => 'Search failed' ], 500);
    }
  }

  // Provide structured response for deleteTransfer to avoid parse errors
  if ($action === 'deleteTransfer') {
    st_json_response([
      'success' => false,
      'error' => 'Delete is not supported from this legacy page. Please use the Stock module.'
    ], 400);
  }
    ...
    switch ($action) {
      case 'markReadyForDelivery': {
          ...
          if (!($result->success ?? false)) {
            ...
            // ---------- GET phase ----------
            // Hand off to CIS module READY view; preserve transfer and packonly flags.
            // This keeps legacy URLs working while using the consolidated Pack page built in the module.

            $_GET['module'] = 'transfers/stock';
            $_GET['view']   = 'pack_ready';
            // Ensure the transfer id is visible to the module
            if ($tid > 0) { $_GET['transfer'] = (string)$tid; }
            // PACKONLY respected by the module
            if ($PACKONLY) { $_GET['packonly'] = '1'; }

            // Render through CIS module system

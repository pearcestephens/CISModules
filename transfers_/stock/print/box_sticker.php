...
<?php
/**
 * Compact box sticker label (e.g., 62x29mm) for each box.
 * URL: /modules/transfers/stock/print/box_sticker.php?transfer=123&box=1&boxes=3&w=1.2&p=E20&from=A&to=B&car=nzpost
 */
require_once $_SERVER['DOCUMENT_ROOT'] . '/app.php';
$tid   = (int)($_GET['transfer'] ?? 0);
$box   = (int)($_GET['box'] ?? 1);
...
<head>
  ...
  <title>TX#<?php echo $tid; ?> · <?php echo $box; ?>/<?php echo $boxes; ?></title>
  <style>
    /* 62x29mm (Brother DK-11209) landscape-like; adjust as needed */
    @page { size: 62mm 29mm; margin: 2mm; }
    html, body { margin:0; padding:0; }
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, 'Noto Sans', sans-serif; }
    .wrap { width: 58mm; height: 25mm; position: relative; }
